document.addEventListener("DOMContentLoaded", () => {
        ...
        const obs = new IntersectionObserver((es) =>
            es.forEach((e) => {
                if (e.isIntersecting && !done) {
                    ...
                    const t = setInterval(() => {
                        s = Math.min(target, s + step);
                        el.textContent = s.toLocaleString("pt-BR");
    ...
    if (bmi)
        bmi.addEventListener("submit", (e) => {
            e.preventDefault();
            let h = +bmi.height.value / 100,
                w = +bmi.weight.value;
            let v = w / (h * h);
            document.querySelector(
                "#bmi-result"
            ).textContent = `IMC: ${v.toFixed(
                1
            )} — resultado informativo, não diagnóstico.`;
        });
...
(function () {
    ...
    document
        .querySelectorAll(
            ".media-frame:not(.image-reveal), .ba-card:not(.image-reveal)"
        )
        .forEach((el) => {
        ...
        document.querySelectorAll("[data-tilt]").forEach((card) => {
            card.addEventListener("pointermove", (e) => {
                ...
                card.style.transform = `perspective(900px) rotateX(${(
                    -y * 3
                ).toFixed(2)}deg) rotateY(${(x * 3).toFixed(
                    2
                )}deg) translateY(-5px)`;
            });
    ...
    document
        .querySelectorAll('a[href$=".html"], a[href^="/"]')
        .forEach((link) => {
...
document.addEventListener("DOMContentLoaded", () => {
     ...
     * Duplica os cards.
     *
     * O primeiro conjunto + segundo conjunto
     * ficam exatamente na sequência.
     *
     * Quando a animação chega a -50%,
     * o segundo conjunto está no mesmo ponto
     * visual do primeiro.
     */
    ...
    /*
     * Garante autoplay também nos vídeos originais.
     */
